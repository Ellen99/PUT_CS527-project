    ...
    private static class RunnersFactory {
        ...
        private List<TestWithParameters> createTestsForCartesianParameters(
            ...
            for (Object[] parametersOfSingleTest : cartesians) {
                children.add(createTestWithParameters(testClass, namePattern, i++, parametersOfSingleTest));
            }
            return children;
        }
        ...
        static List<Object[]> generateCartesians(List<Object[]> arrays) {
            ...
            if (!arrays.isEmpty()) {
                ...
                int[] maxIndices = new int[numColumns];

                for (int i = 0; i < numColumns; i++) {
                    maxIndices[i] = arrays.size() - 1;
                }

                while (true) {
                    Object[] product = new Object[numColumns];
                    for (int i = 0; i < numColumns; i++) {
                    ...
                    int incrementIndex = numColumns - 1;
                    while (incrementIndex >= 0 && indices[incrementIndex] == maxIndices[incrementIndex]) {
        ...
        private static boolean isNotDuplicate(Set<List<Object>> set, Object[] array) {
            ...
            return set.add(Arrays.asList(array));
        }

        private List<Object[]> normalizeAllParameters(Iterable<Object> parameters){
            List<Object[]> params = new ArrayList<>();
            for (Object parametersOfSingleTest:parameters) {
                params.add(normalizeParameters(parametersOfSingleTest));
            }
            return params;
        }
        ...
        private List<Runner> createRunnersForParameters(
            ...
            try {
                List<TestWithParameters> tests = createTestsForCartesianParameters(
